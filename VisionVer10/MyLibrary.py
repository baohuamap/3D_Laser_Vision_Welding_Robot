...
class vision():
    def __init__(self):
        self.intrinsic=np.array([[1.42612167e+03, 0.00000000e+00, 9.47753525e+02],
                                [0.00000000e+00, 1.42350603e+03, 5.80233535e+02],
        ...
        self.eye2hand = np.array([[-9.02106742e-01 ,-2.94502342e-01, -3.15391497e-01,  7.38807766e+01],
                                [ 4.25587844e-01, -7.27928315e-01, -5.37582882e-01,  2.08396646e+02],
                                [-7.12629839e-02, -6.19183929e-01,  7.82005658e-01, -1.16456924e+02],
    ...
    def Preprocessing(self,img):
        ...
        closing = thresh
        del blur
        closing = cv.morphologyEx(closing, cv.MORPH_CLOSE, np.ones((7,7),np.uint8), iterations = 0)
        del thresh
        return closing
    ...
    def CrossLaser(self, img):
        ...
        for i in range(300, self.cols - 20):
            a = np.where(roi[0] == i)   # find pixel which value is 255 in each col
            if len(a[0]) == 1:          # 1 pixel = 255 in i th col
    ...
    def CD(self,img):
        ...
        for i in range(50, len(roi[0]) - 20, 1):
