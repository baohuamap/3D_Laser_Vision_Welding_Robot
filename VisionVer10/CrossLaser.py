...
vis = vision()
img = cv.imread("D:\Code\Python_Code\WeldingRobot\VisionVer10\Mycamera\Laser\CrossLaser5.png",0)
img = cv.resize(img, (1080, 720), interpolation = cv.INTER_AREA)
pre = vis.Preprocessing(img)
rows,cols = img.shape
print(rows, cols)
# dialation = cv.dilate(img, np.ones((5,5), np.uint8), iterations=1)
# med = cv.medianBlur(dialation, 5)

_,thresh = cv.threshold(pre,80,255,cv.THRESH_BINARY)
...
_, _, imgpos = vis.CD(thinned)
cv.circle(thinned, (int(imgpos[0]),int(imgpos[1])), 10, 255, 10)
